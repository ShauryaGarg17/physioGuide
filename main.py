...
def list_exercises():
    exercises = [f.replace(".json", "") for f in os.listdir(DATA_FOLDER) if f.endswith(".json")]
    return exercises

def start_exercise(exercise_name):
    os.environ["EXERCISE_NAME"] = exercise_name
    subprocess.run(["python", "live_pose.py"])
...
if __name__ == "__main__":
    ...
    choice = int(input("Select an exercise (number): ")) - 1
    if 0 <= choice < len(exercises):
        selected_exercise = exercises[choice]
        print(f"Starting {selected_exercise}...")
